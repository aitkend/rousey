 ...
 *
 * https://github.com/nprapps/app-template-testing
 *
...
var disqusEndpoint="https://disqus.com/api/3.0/threads/details.jsonp?api_key="+APP_CONFIG.DISQUS_API_KEY+"&thread%3Aident=";var disqus_config;var disqus_shortname;var disqus_identifier;var configureComments=function(){disqus_shortname=APP_CONFIG.DISQUS_SHORTNAME;disqus_identifier=APP_CONFIG.PROJECT_SLUG+'-'+APP_CONFIG.DISQUS_UUID;};var loadComments=function(){var auth=parseAuthCookie();if(typeof auth.dt!=='undefined'){$('#login-overlay-link').hide();$('#logout-overlay-link').show();disqus_config=function(){this.page.remote_auth_s3=auth.dm+' '+auth.dh+' '+auth.dt;this.page.api_key='K5ANvxVxS7meX7au7vJpUpqIgFqQcDBEH8q39Z8N750SFmBhaOLTsShueMWid956';this.callbacks.onNewComment=[function(){_gaq.push(['_trackEvent',APP_CONFIG.PROJECT_SLUG,'new-comment']);trackComment();}];};}var dsq=document.createElement('script');dsq.type='text/javascript';dsq.async=true;dsq.src='//'+disqus_shortname+'.disqus.com/embed.js';(document.getElementsByTagName('head')[0]||document.getElementsByTagName('body')[0]).appendChild(dsq);};var getCommentCount=function(callback){$.ajax({url:disqusEndpoint+APP_CONFIG.PROJECT_SLUG+"-"+APP_CONFIG.DISQUS_UUID+"&forum="+disqus_shortname,dataType:"jsonp",success:function(data){callback(data.response.posts);}});};var getAuthCookie=function(){var allCookies=document.cookie.split(';');for(var i=0;i<allCookies.length;i++)if($.trim(allCookies[i].split('=')[0])=='at')return decodeURIComponent($.trim(allCookies[i].split('=')[1]));return null;};var parseAuthCookie=function(){var cookie=getAuthCookie();if(!cookie)return{};var data=cookie.split('&');var parsed={};for(pair in data){var bits=data[pair].split('=');var key=bits[0];var val=bits[1];if(key!='email')val=val.replace(/\+/g," ");parsed[key]=decodeURIComponent(val);}return parsed;};$(configureComments);
var SHARE=(function(){var $shareModal=null;var $clippy=null;var $email=null;var $facebook=null;var $twitter=null;var setup=function(){$shareModal=$('#share-modal');$clippy=$shareModal.find('.clippy');$email=$shareModal.find('.email-friend a');$facebook=$shareModal.find('.facebook-share a');$twitter=$shareModal.find('.twitter-share a');$shareModal.on('shown.bs.modal',onShareModalShown);$shareModal.on('hidden.bs.modal',onShareModalHidden);$email.on('click',onEmailClick);$facebook.on('click',onFacebookClick);$twitter.on('click',onTwitterClick);ZeroClipboard.config({swfPath:'js/lib/ZeroClipboard.swf'});var clippy=new ZeroClipboard($clippy);clippy.on('ready',function(readyEvent){clippy.on('aftercopy',onClippyCopy);});};var onShareModalShown=function(){ANALYTICS.openShareDiscuss();};var onShareModalHidden=function(){ANALYTICS.closeShareDiscuss();};var onClippyCopy=function(){alert('Copied to your clipboard!');ANALYTICS.copySummary();};var onEmailClick=function(){ANALYTICS.clickEmail('share-discuss');};var onFacebookClick=function(){ANALYTICS.clickFacebook('share-discuss');};var onTwitterClick=function(){ANALYTICS.clickTweet('share-discuss');};return{'setup':setup};}());
var $commentCount=null;var onDocumentLoad=function(e){$commentCount=$('.comment-count');renderExampleTemplate();getCommentCount(showCommentCount);SHARE.setup();};var renderExampleTemplate=function(){var context=$.extend(APP_CONFIG,{'template_path':'jst/example.html','config':JSON.stringify(APP_CONFIG,null,4),'copy':JSON.stringify(COPY,null,4)});var html=JST.example(context);$('#template-example').html(html);};var showCommentCount=function(count){$commentCount.text(count);if(count>0)$commentCount.addClass('has-comments');if(count>1)$commentCount.next('.comment-label').text('Comments');};$(onDocumentLoad);